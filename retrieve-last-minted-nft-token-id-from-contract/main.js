(async function () {
    try {
        ...
        const tokenId = await ((async function next(i) {
            if (i < 0) return

            const { hash, data } = history[i]

            const { name } = iface.getFunction(data.slice(0, 10))
            ...
            if (name === 'mint') {
                ...
                if (status === 1) {
                    ...
                    return decodedArgs.tokenId.toNumber()
                }
            }
